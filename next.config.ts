...
export default withSentryConfig(nextConfig, {
  ...
  // NOTE: a `webpack: { treeshake, automaticVercelMonitors }` block used to live here.
  // It was removed: `next build` runs on Turbopack (Next 16 default) and Sentry's
  // `webpack` options have no effect under Turbopack, so it was dead config.
})
