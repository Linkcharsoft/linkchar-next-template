...
- **Console output is stripped in production**: `compiler.removeConsole` removes every `console.*` call (client *and* server bundles — SWC has no per-runtime switch), and `logging.fetches.fullUrl` is dev/staging-only. Errors still reach Sentry via `captureException`; nothing depended on a console integration. If a project needs server-side breadcrumbs in CloudWatch, use `Sentry.captureMessage`/`logger`, not `console.error`.
