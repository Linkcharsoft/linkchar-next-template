...
- **Never log an error with `console` — report it.** The template's own `src/` is console-free: every swallowed `catch` calls `Sentry.captureException(error)` (`proxy.ts`, `utils/auth.ts`, the login route handler, `customFetch`, and the auth screens). Use `Sentry.logger.info` for an expected condition that is not a failure — a refresh token expiring on schedule is the reference case, and reporting it as an error would eat the quota. `enableLogs` is already on outside development in all three runtimes, so no extra setup is needed.
