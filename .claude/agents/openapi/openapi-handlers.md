...
4. `CLAUDE.md` — the project root doc. **Search for the heading `## Swagger/OpenAPI-to-Code Workflow`** (do NOT seek a line number — this pointer used to read "around lines 194-204" and the section had already moved to 226; `CLAUDE.md` is edited often, so any pinned range rots). That section documents the `Type` suffix on every model interface and the `interface` (not `type alias`) convention for object shapes. Also the **"API Layer"** section confirms the wrapper contract you must respect.
