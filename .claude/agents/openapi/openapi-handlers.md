...
4. `CLAUDE.md` — the project root doc. **Search for the heading `## Swagger/OpenAPI-to-Code Workflow`, never a line number** — the file is edited often and any pinned range rots. That section documents the `Type` suffix on every model interface and the `interface` (not `type alias`) convention for object shapes. Also the **"API Layer"** section confirms the wrapper contract you must respect.
