...
1. **Read the primitive's source JSX file** at the path the parent gave. Study the inline `style={{}}` objects and the `variants` map: real padding per side, `borderRadius` (often `var(--radius)` or `calc(var(--radius) + Npx)`), border widths, `fontFamily`/`fontSize`/`fontWeight`, hover/active handlers (`onMouseDown`/`onMouseEnter` toggling transform/shadow), and disabled styling. The parent's textual description is a HINT; the JSX is the spec.
2. **Token validation gate** — scan every value the primitive uses (colors as hex or `var(--x)`, font sizes, weights, radii, breakpoints) and cross-check against `tailwind.config.js`. The prototype's CSS vars (`--accent`, `--ink`, `--soft`, `--line`, `--radius`) correspond to tokens the parent added in Step 1 — map them (e.g. `var(--accent)` → the `brand-*`/`accent-*` token). If ANY value has no corresponding token, emit `STOP-BLOCKING / category: TOKENS_MISSING / next_agent: claude-design-tokens` (format below) so the parent runs tokens first. NEVER hardcode hex or arbitrary `text-[Npx]`.
