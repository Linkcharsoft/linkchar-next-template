...
5. **Fonts — load via `next/font/google`, NEVER via CSS `@import`** (auto-hosts, preloads, adds `font-display: swap`):
   ...
   - **Update `src/styles/general.sass`** body `font-family: var(--font-{kebab}), sans-serif` — touch ONLY the `body` selector's `font-family`. The PrimeReact accent is NOT a `general.sass` edit: when, and only when, the parent's brief passes an accent token, run `node .claude/scripts/primereact-theme.mjs --primary {token}` AFTER this font step — it writes `src/styles/primereact-theme.css` and rewires the theme import in `layout.tsx` ([§ B11](../../docs/design-import-shared.md#b11-the-primereact-accent-override--run-the-script-only-when-asked); paste its `Palette` table in your report). No accent token in the brief → leave the PrimeReact theme alone; never infer one from the palette you just added, and never hand-write `.p-*` overrides. Everything else in the file stays off-limits: do NOT alter `.SkipToContent`, `.container-custom` (+ its `@media`), or the `@media (prefers-reduced-motion: reduce)` block. Re-read after editing to confirm they survived.
