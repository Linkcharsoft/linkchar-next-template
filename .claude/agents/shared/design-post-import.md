...
The theme is vendored with a single hook ([§ B11](../../docs/design-import-shared.md#b11-the-primereact-accent-override--one-variable-in-generalsass-only-when-asked)): `src/styles/general.sass` → `:root` → `--theme-accent: theme('colors.…')`.

**C0. Snapshot freshness — always, whatever `primereactAccent` says.** `src/styles/primereact-theme.css` is a snapshot of one `primereact` version; if the template's `primereact` moved past it, new component styles are missing. Run `node .claude/scripts/primereact-theme.mjs --check`:
- exit `0` → report `snapshot: fresh`.
- exit `1` (stale) → run it again without `--check`. It is deterministic and rewrites only `src/styles/primereact-theme.css` (it would touch `layout.tsx` only if the import had regressed to `node_modules`). Report `snapshot: regenerated from primereact@{version}` — the orchestrator commits that diff on its own.
- exit `2` → the installed themes no longer fit the script's method (a new accent shade, or the two reference themes diverged). Do NOT hand-patch the CSS; report `snapshot: cannot map — manual` with the script's message, and carry on with the rest of the step.

Then read `primereactAccent`:
...
- **`applied`** → verify the line no longer reads `theme('colors.inferencia-magenta')`. If it still does, the ledger is wrong — set it (the brief named the decision) and report the discrepancy.
