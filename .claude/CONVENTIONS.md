...
| `StoryblokBridge` | `components/StoryblokBridge/StoryblokBridge.tsx` | Loads the Storyblok Visual Editor bridge and refreshes the route on every edit. Render it only behind `preview` (`isPreview()` from `src/api/storyblok.ts`) in a `page.tsx`; never in a screen. |
