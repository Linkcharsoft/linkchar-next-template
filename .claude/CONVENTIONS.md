...
| `ContactForm` | `components/ContactForm/ContactForm.tsx` | Reference contact form wired to `/api/contact` (Resend): Formik + Yup, `InputContainer` fields, honeypot, optional Turnstile, optional attachments driven by `CONTACT_FORMS`. Ships UNMOUNTED — the `init-add-resend` agent adapts fields / copy / language and mounts it. Optional `className`. |
...
- **`inferencia-magenta` (`#FE00FE`) is the template's own brand accent**, template-shipped like `surface-*` (so it never appears in `design-tokens-map.md`): it drives `--theme-accent` in `general.sass` and the Waves by default. A project replaces the *references* with its own accent token; it never REUSEs `inferencia-magenta` for a design colour and never deletes it from `tailwind.config.js`.
...
- **Theme accent.** The `lara-light-blue` theme is vendored at `src/styles/primereact-theme.css` (imported by `layout.tsx`) with its accent slots turned into `var(--theme-accent…)`. To re-skin every component set ONE variable in `src/styles/general.sass` — `:root` → `--theme-accent: theme('colors.{accent token}')` (template default `theme('colors.inferencia-magenta')`); hover / active / ring / highlight / 50…900 derive from it via `color-mix()` and can each be pinned the same way. The file carries no `font-family`, so PrimeReact components inherit the page font. Never edit the vendored file, never hand-map `.p-*` selectors **in `general.sass`**, never override PrimeReact's own `--primary-color` (read by no rule). Per-instance `.p-*` overrides in a wrapper component's own colocated `.sass` are the exception and the intended channel — `ToastNotifications.sass` re-skins the toast surface that way, since severities are state signals the accent hook deliberately does not reach. It is generated by `.claude/scripts/primereact-theme.mjs` — re-run it after a `primereact` upgrade (`--check` reports a stale snapshot; the design-import flows run that check at Step 7 and regenerate it, so an upgrade in the template is caught on the next import). Contract for the import agents: [`design-import-shared.md` § B11](docs/design-import-shared.md#b11-the-primereact-accent-override--one-variable-in-generalsass-only-when-asked).
...
- **`ToggleButton` and `Steps` cannot be server-rendered under the global Tailwind passthrough** (`pt: Tailwind` in `ProvidersContainer`): their preset sections read `state.focused` / `parent.state.activeIndex` before those exist, so `next build` fails at prerender with `Cannot read properties of undefined (reading 'focused' | 'state')` (measured on PrimeReact 10.9.9). Use `SelectButton` / `InputSwitch` and `TabView` / `Timeline` instead, or render them client-only via `dynamic(…, { ssr: false })`.
