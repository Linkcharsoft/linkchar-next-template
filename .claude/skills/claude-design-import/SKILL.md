...
> **How much of a multi-page design a Standalone actually carries — measured, because this used to be stated wrongly.** This section previously claimed the Standalone "carries only ONE page and silently drops the rest". Both halves are false. Measured on two real exports: **StreetBuild's Standalone carried 8 pages complete** (full link closure, no missing siblings), while **Tercer Milenium's carried 1 of 5**. So the count varies by export and you cannot predict it — but you are never left guessing, because when pages ARE missing `unpack.mjs` **aborts** and names them exactly (see [§ Step 0 partial-export](#step-0--unpack-the-export-deterministic-no-llm)). The failure is loud, not silent.
>
...
>
> **It now DETECTS and reports the gap rather than truncating silently.** On the archive path the entry's own relative same-dir `.html` links are checked against disk, and any that exist but aren't covered by a detected route produce a `notes` WARNING naming them. Deliberately a detector, not a closure: `.html` is far more common than `.dc.html`, so a BFS would happily pull in unrelated pages — a loud "I did not import these" beats a heuristic that over-collects. **The count is a FLOOR** (one level deep, entry links only): on Anodal's `index.html` it finds 7 while the site really has 9 others, because `producto` and `novedad` are linked only from sub-pages. Import each missing page separately, or accept the gap knowingly.
>
> **A sample DOES exist, contrary to what this said before** — Anodal's archive ships BOTH `Anodal-completo.html` (the 10-route SPA the README names) AND the same site as 10 cross-linked sibling pages (`index.html`, `vivienda.html`, …). So if the closure is ever built, it has a ready-made test with an ideal oracle: pointing the extractor at `index.html` must yield the SAME 10 routes as the SPA does. The earlier claim that "no sample has exercised it" was wrong, and was what justified not building it.
>
> **⚠️ This paragraph used to cite Anodal as validating the vanilla path — it did the opposite.** The claim was that Anodal's README "named a single combined `Anodal-completo.html`, so it didn't matter there." That file is not a single page: it is a **client-side hash-router SPA holding all 10 routes** as `<script type="text/template" data-route="…">` blocks. `parseVanilla` hard-coded `screens: [{key:'index'}]` and never looked at the markup, so `inventory.json` reported `screens=1` for a 10-route site — and the one design cited as proof the limitation was harmless was in fact its worst case. Measured and fixed 2026-07-20: the extractor now scans for those template blocks, writes one `source/route.{key}.markup.html` per route, and sets `navModel=multi-page`. **The lesson generalizes past this bug: "format X was validated on design Y" is worth nothing unless someone checked that Y actually exercises X's hard part.**
...
>
> Getting this wrong inverts the site — measured on the StreetBuild archive: the README named `Streetbuild Home Short.dc.html` (16KB, in-degree 0) while the real home (40KB) is linked by all 8 siblings, so `/` became an unlinked variant and every sibling's nav "home" link pointed at `/streetbuild-home`. **Nothing downstream catches it**: each page renders correctly on its own, so lint, type-check, `pnpm build` and `design-validation` — runtime sweep included — all pass on a site with the wrong topology. This checkpoint is the only place it can be caught.
>
> **An absent WARNING is not proof the entry is right, only that it is not an orphan.** A hub-and-spoke landing legitimately has in-degree 0 on its home (measured: Tercer Milenium — the landing links to its 4 sub-pages and none link back), which is exactly why the check requires a competing hub before firing. The floor is calibrated on the two multi-page samples available; widen it if a third disagrees.
...
> **The gate can only be as complete as the tree you point it at — a missing file reads as a missing FEATURE.** Measured on the Anodal run: `parseVanilla` wrote only the `<body>` and the `<style>` blocks, so the Google-Fonts `<link>` never reached `source/`. Asked to re-derive the fonts, the auditor correctly reported that nothing in the source declared any — and I concluded `brandFonts` was fabricated and nearly shipped the design with its real webfont dropped. Two passes agreeing, both reading the same amputated evidence. (`index.head.html` is now written, but the failure mode is general.) So when the gate reports an ABSENCE — "no fonts declared", "no @media", "no images" — check that the file which would carry it is actually in the tree before believing it. An absence is only evidence if the thing had somewhere to be.
