...
> **How much of a multi-page design a Standalone actually carries — measured.** It varies by export and you cannot predict it: StreetBuild's Standalone carried **8 pages complete** (full link closure, no missing siblings), Tercer Milenium's carried **1 of 5**. You are never left guessing, though — when pages ARE missing `unpack.mjs` **aborts** and names them exactly (see [§ Step 0 partial-export](#step-0--unpack-the-export-deterministic-no-llm)). The failure is loud, not silent.
>
...
>
> **A ready-made test sample exists if the closure is ever built** — Anodal's archive ships BOTH `Anodal-completo.html` (the 10-route SPA the README names) AND the same site as 10 cross-linked sibling pages (`index.html`, `vivienda.html`, …). That gives an ideal oracle: pointing the extractor at `index.html` must yield the SAME 10 routes as the SPA does. **Note the general trap it illustrates: "format X was validated on design Y" is worth nothing unless someone checked that Y actually exercises X's hard part.**
