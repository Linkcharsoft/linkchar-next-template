...
2. **Strip the guard block from `.husky/pre-commit`, and ONLY that block.** Delete from the leading `# Block commits until the project is initialized…` comment through the `fi` that closes the `if node -e '…'` test. **Keep everything after that `fi` exactly as it is** — those lines are the hook's real work (the staged-file lint and the whole-project type-check), not part of the guard. Read the file and delete the range; do not retype the remainder.
   ...
   > **Why this step describes a deletion instead of quoting the replacement.** It used to say "restore it to its original single line, `pnpm run check || exit 1`" — and that had already gone stale: the hook now runs `pnpm exec lint-staged` (staged files only) plus `pnpm run type-check`. Following the old wording verbatim silently traded the lint-staged optimization for a slower whole-repo lint, in a file the developer has no reason to re-read afterwards. Same failure as the `engines` versions in [`CLAUDE.md`](../../../CLAUDE.md) and the `container-custom` tiers in [`CONVENTIONS.md`](../../CONVENTIONS.md#global-container): **a copy of file content living in a doc rots, and the doc is believed anyway.** So the rule here is the deletion boundary — which is stable — and `.husky/pre-commit` stays the only source of truth for what the hook runs. (Caught 2026-07-21 while initializing a real product.)
